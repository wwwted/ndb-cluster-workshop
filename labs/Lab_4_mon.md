...
#### memory_per_fragment
Memory usage by individual fragments. This table can be used for investigating memory usage for all "user defined" tables with query below. If you want to filter out specific schemas or tables add *where* clause to statement and filter on `fq_name`, format of *fq_name* is \<schema\>/def/\<table\> as seen for our test table below.
```
